...
//    }
    ...
    public void setScores(List<bees.Point> scores) {
        this.scores = scores;
        invalidate();
        this.repaint();
    }

    public List<bees.Point> getScores() {
        return scores;
    }
	private String myRound(String x){
		int thirdDigitIndex = x.indexOf('.')+3;
		byte thirdDigit = Byte.parseByte(String.valueOf(x.charAt(thirdDigitIndex)));
		if(thirdDigit < 5) return x.substring(0,thirdDigitIndex);
		else{	
			String newNumber = String.valueOf(Double.parseDouble(x.substring(0,thirdDigitIndex))+0.01);
			thirdDigitIndex = x.indexOf('.')+3;

			return newNumber.substring(0,thirdDigitIndex);
		}
	}
